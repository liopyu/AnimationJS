...
public abstract class AbstractServerEmotePlay<P> extends ServerEmoteAPI {
    ...
    public static HashMap getLoadedEmotes() {
        return getInstance().getLoadedEmotesImpl();
    }
    public static void setPlayerPlayingEmote(UUID player, @Nullable KeyframeAnimation emote) {
        AbstractServerEmotePlay.getInstance().setPlayerPlayingEmoteImpl(player, emote, false);
    }
    @Override
    public void setPlayerPlayingEmoteImpl(UUID player, @Nullable KeyframeAnimation emoteData, boolean isForced) {
        if (emoteData != null) {
